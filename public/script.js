  ...
  if (loginForm) {
    loginForm.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        const data = await response.json(); // front recebendo token
  ...
  async function loadDashboardData() {
    try {
      ...
      const countsResp = await fetch(
        'http://localhost:3000/api/dashboard/counts',
        {
          headers: {
      ...
      if (countsResp.ok) {
        ...
        // Itens em estoque
        if (inStockEl) {
          inStockEl.textContent = counts.inStock ?? counts.total ?? '0';
        }

        if (discardEl) {
          discardEl.textContent = counts.discard ?? '0';
        }
      }
...
const [catResp, statusResp] = await Promise.all([
  ...
  fetch(
    'http://localhost:3000/api/dashboard/equipamentos/category',
    {
      headers: {
        Authorization: `Bearer ${token}`
      }
  ...
  fetch(
    'http://localhost:3000/api/dashboard/equipamentos/status',
    {
      headers: {
        Authorization: `Bearer ${token}`
      }
